    ...
    for (let r = row; r >= 0; r--) {
      for (let c = col; c >= 0; c--) {
        ...
        if (candidate?.merge) {
          ...
          if (row >= r && row < r + candidate.merge.rowSpan &&
              col >= c && col < c + candidate.merge.colSpan) {
            return { row: r, col: c, rowSpan: candidate.merge.rowSpan, colSpan: candidate.merge.colSpan }
          }
        }
      }
    }
